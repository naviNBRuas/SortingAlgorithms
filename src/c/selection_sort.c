...
void selection_sort(int arr[], int n, FILE* trace_file) {
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = i + 1; j < n; j++) {
            ...
            fprintf(trace_file, "{\"type\": \"compare\", \"indices\": [%d, %d]}", min_idx, j);
            first_step = 0;
        ...
        if (min_idx != i) {
            ...
            fprintf(trace_file, "{\"type\": \"swap\", \"indices\": [%d, %d]}", i, min_idx);
            first_step = 0;
            int temp = arr[i];
            arr[i] = arr[min_idx];
            arr[min_idx] = temp;
...
int main(int argc, char *argv[]) {
    ...
    selection_sort(arr, n, trace_file);
