...
#include <assert.h>
#include <string.h>
#include "../../src/c/batcher_odd_even_merge_sort.h"

void batcher_odd_even_merge_sort_test_wrapper(int arr[], int n) {
    FILE* dummy_trace_file = fopen("dummy_trace.json", "w");
    if (dummy_trace_file == NULL) {
        perror("Error opening dummy trace file");
        exit(1);
    }
    batcher_odd_even_merge_sort(arr, n, dummy_trace_file);
...
void test_empty_array() {
    ...
    int expected[] = {};
    int n = 0;
    batcher_odd_even_merge_sort_test_wrapper(arr, n);
