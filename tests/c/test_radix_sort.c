...
void radix_sort_test_wrapper(int arr[], int n) {
    ...
    radix_sort(arr, n, dummy_trace_file);
...
void test_radix_specific_array() {
    int arr[] = {170, 45, 75, 90, 802, 24, 2, 66};
    int expected[] = {2, 24, 45, 66, 75, 90, 170, 802};
    int n = 8;
    radix_sort_test_wrapper(arr, n);
    assert(compare_arrays(arr, expected, n));
    printf("Test radix specific array passed.\n");
}
...
int main() {
    ...
    test_random_array();
    test_radix_specific_array(); // Add radix specific test
    return 0;
}
