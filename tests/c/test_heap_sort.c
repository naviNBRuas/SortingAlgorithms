...
void heap_sort_test_wrapper(int arr[], int n) {
    ...
    heap_sort(arr, n, dummy_trace_file);
...
void test_heap_specific_array() {
    int arr[] = {4, 10, 3, 5, 1};
    int expected[] = {1, 3, 4, 5, 10};
    int n = 5;
    heap_sort_test_wrapper(arr, n);
    assert(compare_arrays(arr, expected, n));
    printf("Test heap specific array passed.\n");
}
...
int main() {
    ...
    test_random_array();
    test_heap_specific_array(); // Add heap specific test
    return 0;
}
