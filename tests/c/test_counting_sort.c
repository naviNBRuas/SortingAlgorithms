...
void counting_sort_test_wrapper(int arr[], int n) {
    ...
    counting_sort(arr, n, dummy_trace_file);
...
void test_empty_array() {
    ...
    int expected[] = {};
    int n = 0;
    counting_sort_test_wrapper(arr, n);
...
void test_counting_specific_array() {
    int arr[] = {9, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5};
    int expected[] = {1, 1, 2, 3, 4, 5, 5, 5, 6, 9, 9};
    int n = 11;
    counting_sort_test_wrapper(arr, n);
    assert(compare_arrays(arr, expected, n));
    printf("Test counting specific array passed.\n");
}
...
int main() {
    ...
    test_random_array();
    test_counting_specific_array(); // Add counting specific test
    return 0;
}
